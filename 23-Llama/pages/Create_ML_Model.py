...
import joblib
from utils.utils import read_data_set, get_score, split_train_test_data
from utils.siteloader import split_data_pdf, create_embeddings, create_embeddings_from_dataframe
...
with tabs[0]:
    ...
    if button:
        with st.spinner('Wait for it...'):
            our_data=read_data_set(data)
            embeddings=create_embeddings()
            st.session_state['cleaned_data'] = create_embeddings_from_dataframe(our_data,embeddings)
        st.success('Done!')
...
with tabs[1]:
    ...
    if button:
            with st.spinner('Wait for it...'):
                result =split_train_test_data(st.session_state['cleaned_data'])
                sentences_train, sentences_test, labels_train, labels_test = result
                st.session_state['sentences_test'] = sentences_test
                st.session_state['labels_test'] = labels_test
                ...
                st.session_state['svm_classifier'].fit(sentences_train, labels_train)
            st.success('Done!')
...
with tabs[3]:
    ...
    if button:
        ...
        with st.spinner('Wait for it...'):
             joblib.dump(st.session_state['svm_classifier'], 'model/modelsvm.pk1')
        st.success('Done!')